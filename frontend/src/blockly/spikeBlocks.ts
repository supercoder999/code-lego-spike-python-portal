...
// Get the Python generator
const pythonGenerator = (Blockly as any).Python || (Blockly as any).python?.pythonGenerator;
...
export function registerGenerators(): void {
  ...
  pythonGenerator.forBlock['spike_drivebase_init'] = function (block: Blockly.Block) {
    const leftPort = block.getFieldValue('LEFT_PORT');
    const rightPort = block.getFieldValue('RIGHT_PORT');
    ...
    return (
      `left_motor = Motor(${leftPort})\n` +
      `right_motor = Motor(${rightPort}, Direction.COUNTERCLOCKWISE)\n` +
      `drive_base = DriveBase(left_motor, right_motor, ${wheelDiameter}, ${axleTrack})\n`
    );
  };
